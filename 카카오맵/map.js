...
console.log("위도:"+latlng.getLat()+" 경도:"+latlng.getLat());  
...
kakao.maps.event.addListener(marker, 'click', function(mouseEvent) {  
        ...
        if (index !== -1) {
            ...
            markers.splice(index, 1); // 배열에서 해당 마커 제거
...
var idx=0;
if ( idx >= 0 && idx < arr.length) { // 인덱스가 유효한지 확인
    for (var i = idx + 1; i < arr.length; i++) {
        arr[i - 1] = arr[i]; // 현재 요소를 이전 요소로 이동
    }
}
}
...
//marker.setMap(map); // 지도 위에 마커를 
listmarkers.push(marker);  // 배열에 생성된 마커를 추가합니다
